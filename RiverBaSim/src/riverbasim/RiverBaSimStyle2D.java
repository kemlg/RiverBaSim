...
/**
 * @author Luis Oliva
 * Based on the work done by Eric tatara
 */
public class RiverBaSimStyle2D extends StyledDisplayLayer2D
{
	...
	public RiverBaSimStyle2D(Style2D s, RepastCanvas2D canvas) {
		super(s, canvas);
	...
	public static String getLabel(EditedStyleData style, Object object) {
		...
		try {
			...
			if (method.getReturnType().equals(String.class))
				...
				label = (method.invoke(object)).toString();
	...
	public static float getMethodFloat(String methodName, Object object) {
		try {
			...
			if (method.getReturnType().equals(Amount.class))
				return (float) ((Amount) method.invoke(object)).getEstimatedValue();

			return ((Number) method.invoke(object)).floatValue();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return 0;
